...
internal class ConnectMobileObjectWithDriverUseCase @Inject constructor(
    private val repository: MobileObjectRepository,
    @Background backgroundScheduler: Scheduler,
    @Foreground foregroundScheduler: Scheduler
) : CompletableUseCase<ConnectMobileObjectWithDriverUseCase.Params>(backgroundScheduler, foregroundScheduler) {
    override fun generateCompletable(input: Params?): Completable {
        requireNotNull(input) { "Params can't be null" }
        return repository.connectWithDriver(input.mobileObject, input.driver)
    }

    class Params(
        val mobileObject: MobileObject,
        val driver: MobileObjectDriver
    )
}