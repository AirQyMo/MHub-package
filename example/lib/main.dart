...
import 'package:mobile_hub_plugin/mobile_hub_plugin.dart'; // Ensure this path is correct
...
class _MobileHubHomePageState extends State<MobileHubHomePage> {
  final MobileHubPlugin _mobileHubPlugin = MobileHubPlugin();
  ...
  void _listenToPluginStreams() {
    ...
    _mobileHubPlugin.onSensorDataReceived.listen((sensorData) {
      setState(() {
        _sensorDataMessages.add('Sensor Data: ${sensorData.toString()}');
      });
  ...
  Future<void> _connectToFirstDiscoveredBleDevice() async {
    if (_discoveredBleDevices.isNotEmpty) {
      // Assuming the device ID is the part after "ID: " in the stored string
      String firstDeviceId = _discoveredBleDevices.first.split('ID: ')[1];
      bool connected = await _mobileHubPlugin.connectBleDevice(firstDeviceId);
      if (connected) {
        _showSnackBar('Attempting to connect to $firstDeviceId');
        // Automatically subscribe to sensor data upon successful connection
        bool subscribed = await _mobileHubPlugin.subscribeToSensorData(firstDeviceId);
        if (subscribed) {
          _showSnackBar('Subscribed to sensor data for $firstDeviceId');
        } else {
          _showSnackBar('Failed to subscribe to sensor data for $firstDeviceId');
        }
      } else {
        _showSnackBar('Failed to connect to $firstDeviceId');
      }
    } else {
      _showSnackBar('No BLE devices discovered to connect to.');
    }
  }
            ...
            children: <Widget>[
              ...
              Row(
                children: [
                  ...
                  const SizedBox(width: 10),
                  ElevatedButton(
                    onPressed: _connectToFirstDiscoveredBleDevice,
                    child: const Text('Connect to 1st BLE Device'),
                  ),
                ],
              ),
